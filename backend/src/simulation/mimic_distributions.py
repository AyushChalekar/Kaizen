...
class MimicIVEmpiricalEngine:
    ...
    def __init__(self, base_arrival_rate: float = 5.71, unique_patients: int = 75000, seed: int = 42):
        ...
        # Create a finite pool of patients to simulate readmissions
        self.patient_pool = np.arange(100000, 100000 + unique_patients)
        # Assign fixed baseline traits to patients to ensure consistency across readmissions
    ...
    def sample_encounter_features(self, stay_id: int, arrival_dt: datetime) -> Dict[str, Any]:
        ...
        # Vitals simplified for brevity, maintaining logical bounds
        if triage_acuity <= 2:
        ...
        # Realistic MIMIC-IV disposition and care unit decoupling
        if triage_acuity == 1:
        ...
        # Introduce stochastic deterioration/escalation independent of initial disposition plan
        ...
        if disposition == "Ward" and self.rng.random() < 0.08:
            icu_transfer_flag = 1 # Deteriorated on the floor
